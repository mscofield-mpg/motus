...
#Load detections 
COPO.detect<-read_csv("~/Library/CloudStorage/Egnyte-mpgcloud/Shared/Workspace/Teams/Avian Science/Motus/Project 213 Data/Processed Detection Data/COPO/8.31.2024/COPO_tag_detections_FILTERED_8.31.24.csv")
...
#then force tz to Mountain time
...
####Import Night Data (temporal)####
Night.dat <- read_csv("~/Library/CloudStorage/Egnyte-mpgcloud/Shared/Workspace/Teams/Avian Science/Motus/Project 213 Data/Projects/COPO_Lunar Breeding Season/Moon Times 2024.csv", 
                            col_types = cols(Night_ID = col_number(), 
                                             Night_Start = col_datetime(format = "%m/%d/%y %H:%M"), 
                                             Night_End = col_datetime(format = "%m/%d/%y %H:%M"), 
                                             date = col_date(format = "%m/%d/%y"), 
                                             moonrise = col_datetime(format = "%m/%d/%y %H:%M"),
                                             moonset = col_datetime(format = "%m/%d/%y %H:%M"), 
                                             sunset = col_datetime(format = "%m/%d/%y %H:%M"), 
                                             dusk = col_datetime(format = "%m/%d/%y %H:%M"), 
                                             nauticalDusk = col_datetime(format = "%m/%d/%y %H:%M"), 
                                             nauticalDawn = col_datetime(format = "%m/%d/%y %H:%M"), 
                                             dawn = col_datetime(format = "%m/%d/%y %H:%M"), 
                                             sunrise = col_datetime(format = "%m/%d/%y %H:%M")))
#Prepare Data for joining with Detections
Night.dat <- Night.dat %>%
  mutate(
    Night_Start = force_tz(Night_Start, "America/Denver"),
    Night_End = force_tz(Night_End, "America/Denver"),
    moonrise = force_tz(moonrise, "America/Denver"),
    moonset = force_tz(moonset, "America/Denver"),
...
#calculate the length of Night Temp in minutes
Night.dat<- Night.dat %>%
  mutate(night_mins = abs(as.numeric(difftime(sunset, sunrise, tz = "America/Denver", units = "mins"))))
####
...
####Define Activity_Function####
  #Modify bin length to desired time in minutes
Activity_Function <- function(COPO.act, Bin_Length = "1 min", Activity_Threshold, Night.dat) {
  ...
  # Step 2: Join with Moon.dat
  activity_table <- activity_table %>%
    fuzzy_left_join(Night.dat,
...
#Run the function
results <- Activity_Function(COPO.act, Bin_Length = "1 min", Activity_Threshold, Night.dat)
...
#Save data tables
write_csv(Activity_Table.5b, "~/Library/CloudStorage/Egnyte-mpgcloud/Shared/Workspace/Teams/Avian Science/Motus/Project 213 Data/Projects/COPO_Lunar Breeding Season/Data Tables/Activity_Table_5mins.csv")
write_csv(Activity_Summary.5, "~/Library/CloudStorage/Egnyte-mpgcloud/Shared/Workspace/Teams/Avian Science/Motus/Project 213 Data/Projects/COPO_Lunar Breeding Season/Data Tables/Activity_Summary_5mins.csv")
####
...
#create vector
mfg_ids <- unique(Activity_Summary.1$mfgID)

#Loop through vector and save each plot
for (ID in mfg_ids) {
  plot_data <- filter(Activity_Summary.1, mfgID == ID)
  ...
  ggsave(filename = paste0("Minutes Active by Night_COPO ", ID, ".png"), 
         path = "~/Library/CloudStorage/Egnyte-mpgcloud/Shared/Workspace/Teams/Avian Science/Motus/Project 213 Data/Projects/COPO_Lunar Breeding Season/Figures/8.28.24/1 min", 
         width = 7, 
         height = 5, 
         units = "in")
...
Activity_Summary.8b<- Activity_Summary.8 %>%
  mutate(Bin_Length = 8)
...
write_csv(Activity_Overview, "~/Library/CloudStorage/Egnyte-mpgcloud/Shared/Workspace/Teams/Avian Science/Motus/Project 213 Data/Projects/COPO_Lunar Breeding Season/Data Tables/Activity_Overview Table.csv")
...
#Loop through vector and save each plot
for (ID in mfg_ids) {
  plot_data <- filter(Activity_Overview.sht, mfgID == ID)
  ...
  ggsave(filename = paste0("Minutes Active by Night_Overview_COPO ", ID, ".png"), 
         path = "~/Library/CloudStorage/Egnyte-mpgcloud/Shared/Workspace/Teams/Avian Science/Motus/Project 213 Data/Projects/COPO_Lunar Breeding Season/Figures/8.28.24/Overview", 
         width = 7, 
         height = 5, 
         units = "in")
}
...
#Load lunar variables
  #Code out import line
Moon.dat<- read_csv("~/Library/CloudStorage/Egnyte-mpgcloud/Shared/Workspace/Teams/Avian Science/Motus/Project 213 Data/Projects/COPO_Lunar Breeding Season/Data Tables/Moon Variables 2024.csv")

#reformat POSIXCT variables!!

#reduce variables and join with detection table by time_bin  
Moon.sht<- Moon.dat %>%
  select(time_bin, fraction, phase, angle, altitude,
         night_mins, moon_available, moon_duration)

Activity_Table.5b<- Activity_Table.5 %>% 
  left_join(Moon.sht, by = "time_bin")
